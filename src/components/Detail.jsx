...
/**
 * Detail component for displaying movie or actor information
 * 
 * @component
 * @returns {React.ReactElement} A detail view that displays either:
 * - Actor details (image, name, biography) when `idInterprete` param is provided
 * - Movie details (poster, summary) with a grid of actors when only `idPeli` param is provided
 * 
 * @description
 * This component uses URL parameters to determine what content to display:
 * - `idPeli`: The ID of the movie (required)
 * - `idInterprete`: The index of the actor in the movie's actors array (optional)
 * 
 * Displays error messages if the movie or actor is not found.
 * Includes navigation buttons to return to the previous page or movie list.
 * 
 * @requires useParams - React Router hook to access URL parameters
 * @requires useNavigate - React Router hook for navigation
 * @requires peliculas - Data array containing movie objects with actors
 */
function Detail() {
